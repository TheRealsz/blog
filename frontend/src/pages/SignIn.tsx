...
// Tranformar em componente os inputs?
// Colocar algo no aside?
const SignIn = () => {
    ...
    const handleSignIn = async(e: FormEvent) => {
        e.preventDefault()
        const signInForm = ref.current
        const fullname = signInForm?.fullname.value
                    ...
                    <div className="w-full flex flex-col gap-9 lg:w-8/12 xl:w-7/12">
                        <div className="w-full flex flex-col items-center gap-4 sm:items-start">
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <p className="text-black/50 2xl:text-lg">Fique por dentro das ultimas novidades e noticias do mundo dentro de nosso site!</p>
                            </div>
                        </div>
                        <form className="w-full flex" name="signInForm" ref={ref} onSubmit={handleSignIn}>
                            <div className="w-full flex flex-col gap-4">
                                <div className='w-full flex flex-col'>
                                    <label className='2xl:text-lg'>Nome completo</label>
                                    <input name="fullname" type="text" placeholder="Insira seu nome completo" className='px-3 py-1.5 rounded-md border-gray-300 border-solid border focus:border-violet-600 outline-none 2xl:py-2' />
                                </div>
                                <div className='w-full flex flex-col'>
                                    <label className='2xl:text-lg'>Email</label>
                                    <input name="email" type="email" placeholder="Insira seu email" className='px-3 py-1.5 rounded-md border-gray-300 border-solid border focus:border-violet-600 outline-none 2xl:py-2' />
                                </div>
                                <div className='w-full flex flex-col'>
                                    <label className='2xl:text-lg'>Senha</label>
                                    <div className='w-full flex items-center justify-between text-center'>
                                        <input name='password' type={`${viewPassword == false ? "password" : "text"}`} placeholder='Insira sua senha' className='peer pl-3 py-1.5 w-11/12 rounded-l-md border-gray-300 border-solid border border-r-0 focus:border-violet-600 outline-none 2xl:py-2' />
                                ...
                                <div className="w-full pt-6">
                                    <button type="submit" className="w-full bg-violet-800 py-2 rounded-md text-white hover:bg-violet-600 transition-all outline-none 2xl:text-lg">Cadastrar</button>
                                </div>
                            </div>
                        </form>
                    </div>
